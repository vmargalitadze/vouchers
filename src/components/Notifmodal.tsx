    ...
    <>
      {context?.notifModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-75 overflow-auto">
          <div className="theme-card rounded-lg p-6 w-11/12 max-w-md mx-auto shadow-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold theme-text">
                შეტყობინებები
              </h2>
              <i
                className="fa-solid fa-x text-xl theme-text hover:text-red-500 cursor-pointer transition-colors"
                onClick={() => context?.setNotifModal(false)}
              ></i>
            </div>
            {isLoading ? (
              <div className="flex justify-center items-center h-24">
                <div className="loader theme-text">Loading...</div>{" "}
              ...
              <div>
                {notifications.length > 0 ? (
                  notifications.map((notification: any, index: number) => (
                    <div
                      key={index}
                      className="mb-4 theme-border border-yellow-500 p-2 rounded"
                    >
                  ...
                  <p className="theme-text">No notifications available.</p>
                )}
              </div>
            )}
          </div>
