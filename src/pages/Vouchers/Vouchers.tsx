...
export default function Vouchers() {
  ...
  return (
    <div className="p-3 rounded-xl theme-bg">
      ...
      <div className="theme-border mt-24 border-yellow-500 flex w-[97%] m-auto rounded-xl">
        <div
          className={`w-1/2 text-center cursor-pointer rounded-xl p-3 theme-text ${
      ...
      {focused === "buy" ? (
        <div className="p-5 flex flex-wrap justify-center gap-10">
          {voucherTypes.map((type) => (
            <div
              key={type}
              className="w-1/3 sm:w-full theme-card rounded-md flex flex-col py-5 items-center"
            >
              <h1 className="theme-text">{type}</h1>
              <img
              ...
              <div className="gap-5 flex justify-between">
                <Link to={`/cards/${type}`}>
                  <button className="theme-border border-yellow-600 rounded p-2 theme-text hover:bg-yellow-500 hover:text-black transition-colors">
                    კატალოგის ნახვა
                  </button>
                </Link>
              </div>
            ...
            context?.userInfo?.subscription === 1 ? (
              <>
                <div className="relative w-full max-w-sm h-auto aspect-[3/2] rounded-lg overflow-hidden">
                  ...
                  />
                  <div className="absolute bottom-20 right-8 p-[2px] rounded-sm test text-black">
                  {context.userInfo.username}
                  </div>
                </div>
              </>
            ) : (
              <p className="theme-text mt-20 text-2xl">
                {context?.isLoggined ? (
